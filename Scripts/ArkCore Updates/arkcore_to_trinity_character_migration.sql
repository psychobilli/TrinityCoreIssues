...
INSERT INTO `character_account_map` (`source_guid`, `target_guid`, `source_account_id`, `target_account_id`) SELECT `s`.`guid`, `t`.`guid`, @source_account_id, @target_account_id FROM `characters` `t` JOIN `ark_characters`.`characters` `s` ON `s`.`name` = `t`.`name` AND `t`.`account` = @target_account_id;
...
SELECT `g`.`guildid`, @guildid := @guildid + 1, `source_guid`, `target_guid` FROM `ark_characters`.`guild` `g` JOIN `ark_characters`.`guild_member` `gm` ON `gm`.`guildid` = `g`.`guildid` JOIN `character_account_map` `cam` ON `cam`.`source_guid` = `gm`.`guid`;
...
FROM `ark_characters`.`guild` `G` JOIN `guild_character_map` `gcm` ON `gcm`.`source_guild_id` = `G`.`guildid` WHERE NOT EXISTS (SELECT 1 FROM `guild` WHERE `name` = `G`.`name`);

select *
from guild;
select *
from ark_characters.guild;
select *
from guild_member;
select *
from ark_characters.guild_member;
select *
from guild_rank;
select *
from ark_characters.guild_rank;
...
FROM `ark_characters`.`guild_member` `gm` JOIN `guild_character_map` `gcm` ON `gm`.`guildid` = `gcm`.`source_guild_id`;
...
INSERT INTO `pet_character_map` (`source_pet_id`, `source_character_guid`, `target_character_guid`) SELECT `p`.`id`, `source_guid`, `target_guid` FROM `ark_characters`.`character_pet` `p` JOIN `character_account_map` `cam` ON `cam`.`source_guid` = `p`.`owner`;
...
FROM `ark_characters`.`character_pet` `p` JOIN `pet_character_map` `pcm` ON `pcm`.`source_pet_id` = `p`.`id`;

UPDATE `pet_character_map` `pcm` JOIN `ark_characters`.`character_pet` `p` ON `p`.`id` = `pcm`.`source_pet_id` JOIN `character_pet` `t` ON `t`.`owner` = `pcm`.`target_character_guid` AND `t`.`name` = `p`.`name` SET `pcm`.`target_pet_id` = `t`.`id`;
...
FROM `ark_characters`.`item_instance` `i` JOIN `character_account_map` `cam` ON `cam`.`source_guid` = `i`.`owner_guid` 
JOIN `ark_characters`.`character_inventory` `ci` on `ci`.`guid` = `cam`.`source_guid` AND `ci`.`item` = `i`.`guid`;
...
FROM `ark_characters`.`item_instance` `ii` JOIN `item_character_map` `icm` on `icm`.`source_item_guid` = `ii`.`guid`;
...
INSERT INTO `item_guild_map` (`source_item_guid`, `target_item_guid`, `source_guild_id`, `target_guild_id`) SELECT `ii`.`guid`, @item_id := @item_id + 1, `source_guild_id`, `target_guild_id` FROM `ark_characters`.`item_instance` `ii` 
JOIN `ark_characters`.`guild_bank_item` `gbi` ON `gbi`.`item_guid` = `ii`.`guid` JOIN (SELECT DISTINCT `source_guild_id`, `target_guild_id` FROM `guild_character_map`) `gcm` ON `gcm`.`source_guild_id` = `gbi`.`guildid`;

INSERT INTO `guild_bank_item` (`guildid`, `TabId`, `SlotId`, `item_guid`)
SELECT `guildid`, `TabId`, `SlotId`, `igm`.`target_item_guid`
FROM `ark_characters`.`guild_bank_item` `gbi` JOIN `item_guild_map` `igm` ON `igm`.`source_guild_id` = `gbi`.`guildid`;
...
FROM `ark_characters`.`item_instance` `ii` JOIN `item_guild_map` `igm` on `igm`.`source_item_guid` = `ii`.`guid`;
...
INSERT INTO `mail_character_map` (source_mail_id, target_mail_id, source_character_guid, target_character_guid) SELECT `m`.`id`, @mail_id := @mail_id + 1, `cam`.`source_guid`, `cam`.`target_guid` FROM `ark_characters`.`mail` `m` JOIN `character_account_map` `cam` ON `cam`.`source_guid` = `m`.`sender`;
...
SELECT `mcm`.`target_mail_id`, `messageType`, `stationery`, `mailTemplateId`, `mcm`.`target_character_guid`, 0, `subject`, `body`, `has_items`, `expire_time`, `deliver_time`, `money`, `cod`, `checked` FROM `ark_characters`.`mail` `m` JOIN `mail_character_map` `mcm` ON `mcm`.`source_character_guid` = `m`.`sender`;

-- INSERT account tables
INSERT INTO `account_data` (`accountId`, `type`, `time`, `data`)
SELECT @target_account_id, `type`, `time`, `data` FROM `ark_characters`.`account_data` WHERE `accountId` = @source_account_id;

INSERT INTO `account_instance_times` (`accountId`, `instanceId`, `releaseTime`) 
SELECT @target_account_id, `instanceId`, `releaseTime` FROM `ark_characters`.`account_instance_times` WHERE `accountId` = @source_account_id;

INSERT INTO `account_tutorial` (`accountId`, `tut0`, `tut1`, `tut2`, `tut3`, `tut4`, `tut5`, `tut6`, `tut7`)
SELECT @target_account_id, `tut0`, `tut1`, `tut2`, `tut3`, `tut4`, `tut5`, `tut6`, `tut7` FROM `ark_characters`.`account_tutorial` WHERE `accountId` = @source_account_id;

-- INSERT character tables
INSERT INTO `character_account_data` (`guid`, `type`, `time`, `data`) 
SELECT `cam`.`target_guid`, `type`, `time`, `data`
FROM `ark_characters`.`character_account_data` `cad` JOIN `character_account_map` `cam` ON `cam`.`source_guid` = `cad`.`guid`;

INSERT INTO `character_achievement` (`guid`, `achievement`, `date`)
SELECT `cam`.`target_guid`, `achievement`, `date`
FROM `ark_characters`.`character_achievement` `ca` JOIN `character_account_map` `cam` ON `cam`.`source_guid` = `ca`.`guid`;

INSERT INTO `character_achievement_progress` (`guid`, `criteria`, `counter`, `date`)
SELECT `cam`.`target_guid`, `criteria`, `counter`, `date`
FROM `ark_characters`.`character_achievement_progress` `cap` JOIN `character_account_map` `cam` ON `cam`.`source_guid` = `cap`.`guid`;

INSERT INTO `character_action` (`guid`, `spec`, `button`, `action`, `type`)
SELECT `cam`.`target_guid`, `spec`, `button`, `action`, `type` 
FROM `ark_characters`.`character_action` `ca` JOIN `character_account_map` `cam` ON `cam`.`source_guid` = `ca`.`guid`;

INSERT INTO `character_archaeology_completed` (`guid`, `project`, `time`, `count`)
SELECT `cam`.`target_guid`, `projectId`, `time_last` - `time_first`, `counter_completed`
FROM `ark_characters`.`character_archaeology_completed` `cac` JOIN `character_account_map` `cam` ON `cam`.`source_guid` = `cac`.`guid`;

INSERT INTO `character_archaeology_projects` (`guid`, `branch`, `project`) 
SELECT `cam`.`target_guid`, `branchId`, `projectId`
FROM `ark_characters`.`character_archaeology_current` `cac` JOIN `character_account_map` `cam` ON `cam`.`source_guid` = `cac`.`guid`;
...
-- FROM `ark_characters`.`character_archaeology_digsite` `cad` JOIN `character_account_map` `cam` ON `cam`.`source_guid` = `cad`.`guid`;

INSERT INTO `character_arena_stats` (`guid`, `slot`, `matchMakerRating`)
SELECT `cam`.`target_guid`, `slot`, `matchMakerRating`
FROM `ark_characters`.`character_arena_stats` `cas` JOIN `character_account_map` `cam` ON `cam`.`source_guid` = `cas`.`guid`;
...
FROM `ark_characters`.`character_banned` `cb` JOIN `character_account_map` `cam` ON `cam`.`source_guid` = `cb`.`guid`;
...
FROM `ark_characters`.`character_cuf_profiles` `ccp` JOIN `character_account_map` `cam` ON `cam`.`source_guid` = `ccp`.`guid`;

INSERT INTO `character_currency` (`guid`, `currency`, `total_count`, `week_count`)
SELECT `cam`.`target_guid`, `currency`, `total_count`, `week_count`
FROM `ark_characters`.`character_currency` `cc` JOIN `character_account_map` `cam` ON `cam`.`source_guid` = `cc`.`guid`;
...
FROM `ark_characters`.`character_equipmentsets` `ce`  JOIN `character_account_map` `cam` ON `cam`.`source_guid` = `ce`.`guid`;

INSERT INTO `character_glyphs` (`guid`, `talentGroup`, `glyph1`, `glyph2`, `glyph3`, `glyph4`, `glyph5`, `glyph6`, `glyph7`, `glyph8`, `glyph9`)
SELECT `cam`.`target_guid`, `spec`, `glyph1`, `glyph2`, `glyph3`, `glyph4`, `glyph5`, `glyph6`, `glyph7`, `glyph8`, `glyph9`
FROM `ark_characters`.`character_glyphs` `cg` JOIN `character_account_map` `cam` ON `cam`.`source_guid` = `cg`.`guid`;
...
FROM `ark_characters`.`character_homebind` `ch` JOIN `character_account_map` `cam` ON `cam`.`source_guid` = `ch`.`guid`;

INSERT INTO `character_instance` (`guid`, `instance`, `permanent`)
SELECT `cam`.`target_guid`, `instance`, `permanent` 
FROM `ark_characters`.`character_instance` `ci` JOIN `character_account_map` `cam` ON `cam`.`source_guid` = `ci`.`guid`;
...
FROM `ark_characters`.`character_queststatus` `cq` JOIN `character_account_map` `cam` ON `cam`.`source_guid` = `cq`.`guid`;

INSERT INTO `character_queststatus_daily` (`guid`, `quest`, `time`)
SELECT `cam`.`target_guid`, `quest`, `time`
FROM `ark_characters`.`character_queststatus_daily` `cqd` JOIN `character_account_map` `cam` ON `cam`.`source_guid` = `cqd`.`guid`;

INSERT INTO `character_queststatus_monthly` (`guid`, `quest`)
SELECT `cam`.`target_guid`, `quest`
FROM `ark_characters`.`character_queststatus_monthly` `cqm` JOIN `character_account_map` `cam` ON `cam`.`source_guid` = `cqm`.`guid`;

INSERT INTO `character_queststatus_rewarded` (`guid`, `quest`, `active`)
SELECT `cam`.`target_guid`, `quest`, `active`
FROM `ark_characters`.`character_queststatus_rewarded` `cqr` JOIN `character_account_map` `cam` ON `cam`.`source_guid` = `cqr`.`guid`;

INSERT INTO `character_queststatus_seasonal` (`guid`, `quest`, `event`)
SELECT `cam`.`target_guid`, `quest`, `event`
FROM `ark_characters`.`character_queststatus_seasonal` `cqs` JOIN `character_account_map` `cam` ON `cam`.`source_guid` = `cqs`.`guid`;

INSERT INTO `character_queststatus_weekly` (`guid`, `quest`)
SELECT `cam`.`target_guid`, `quest`
FROM `ark_characters`.`character_queststatus_weekly` `cqw` JOIN `character_account_map` `cam` ON `cam`.`source_guid` = `cqw`.`guid`;

INSERT INTO `character_reputation` (`guid`, `faction`, `standing`, `flags`)
SELECT `cam`.`target_guid`, `faction`, `standing`, `flags`
FROM `ark_characters`.`character_reputation` `cr` JOIN `character_account_map` `cam` ON `cam`.`source_guid` = `cr`.`guid`;

INSERT INTO `character_skills` (`guid`, `skill`, `value`, `max`)
SELECT `cam`.`target_guid`, `skill`, `value`, `max`
FROM `ark_characters`.`character_skills` `cs` JOIN `character_account_map` `cam` ON `cam`.`source_guid` = `cs`.`guid`;
...
FROM `ark_characters`.`character_spell` `cs` JOIN `character_account_map` `cam` ON `cam`.`source_guid` = `cs`.`guid`;

INSERT INTO `character_spell_cooldown` (`guid`, `spell`, `item`, `time`)
SELECT `cam`.`target_guid`, `spell`, `item`, `time`
FROM `ark_characters`.`character_spell_cooldown` `csc` JOIN `character_account_map` `cam` ON `cam`.`source_guid` = `csc`.`guid`;
...
FROM `ark_characters`.`character_stats` `cs` JOIN `character_account_map` `cam` ON `cam`.`source_guid` = `cs`.`guid`;

INSERT INTO `character_talent` (`guid`, `spell`, `talentGroup`)
SELECT `cam`.`target_guid`, `spell`, `spec`
FROM `ark_characters`.`character_talent` `ct` JOIN `character_account_map` `cam` ON `cam`.`source_guid` = `ct`.`guid`;
...
FROM `ark_characters`.`corpse` `c` JOIN `character_account_map` `cam` ON `cam`.`source_guid` = `c`.`guid`;
...
FROM `ark_characters`.`guild_member_withdraw` `gmw` JOIN `character_account_map` `cam` ON `cam`.`source_guid` = `gmw`.`guid`;
...
SELECT `pcm`.`target_pet_id`, `pcm`.`target_character_guid`, `genitive`, `dative`, `accusative`, `instrumental`, `prepositional` FROM `ark_characters`.`character_pet_declinedname` `cpd` JOIN `pet_character_map` `pcm` ON `pcm`.`source_pet_id` = `cpd`.`id`;

INSERT INTO `pet_spell` (`guid`, `spell`, `active`)
SELECT `pcm`.`target_pet_id`, `spell`, `active` FROM `ark_characters`.`pet_spell` `ps` JOIN `pet_character_map` `pcm` ON `pcm`.`source_pet_id` = `ps`.`guid`;

INSERT INTO `pet_spell_cooldown` (`guid`, `spell`, `time`)
SELECT `pcm`.`target_pet_id`, `spell`, `time` FROM `ark_characters`.`pet_spell_cooldown` `psc` JOIN `pet_character_map` `pcm` ON `pcm`.`source_pet_id` = `psc`.`guid`;
...
-- INSERT guild tables
 ...
 -- FROM `ark_characters`.`character_guild_reputation` `cgr` JOIN `guild_character_map` `gcm` ON `gcm`.`source_guild_id` = `cgr`.`guildid` AND `gcm`.`source_character_id` = `cgr`.`guid`;

INSERT INTO `guild_achievement` (`guildId`, `achievement`, `date`, `guids`)
SELECT `gcm`.`target_guild_id`, `achievement`, `date`, `guids` FROM `ark_characters`.`guild_achievement` `ga` JOIN `guild_character_map` `gcm` ON `gcm`.`source_guild_id` = `ga`.`guildid`;
...
FROM `ark_characters`.`guild_achievement_progress` `gap` JOIN `guild_character_map` `gcm` ON `gcm`.`source_guild_id` = `gap`.`guildid`;
...
FROM `ark_characters`.`guild_bank_eventlog` `gbe` JOIN `guild_character_map` `gcm` ON `gcm`.`source_guild_id` = `gbe`.`guildid`;

INSERT INTO `guild_bank_right` (`guildid`, `TabId`, `rid`, `gbright`, `SlotPerDay`)
SELECT `gcm`.`target_guild_id`, `TabId`, `rid`, `gbright`, `SlotPerDay`
FROM `ark_characters`.`guild_bank_right` `gbr` JOIN `guild_character_map` `gcm` ON `gcm`.`source_guild_id` = `gbr`.`guildid`;

INSERT INTO `guild_bank_tab` (`guildid`, `TabId`, `TabName`, `TabIcon`, `TabText`)
SELECT `gcm`.`target_guild_id`, `TabId`, `TabName`, `TabIcon`, `TabText`
FROM `ark_characters`.`guild_bank_tab` `gbt` JOIN `guild_character_map` `gcm` ON `gcm`.`source_guild_id` = `gbt`.`guildid`;
	-- unknown target guild_challenges_completed.guildId
 -- SELECT `gcm`.`target_guild_id`, `challengeId`, `dateCompleted`
 -- FROM `ark_characters`.`guild_challenges_completed` `gcc` JOIN `guild_character_map` `gcm` ON `gcm`.`source_guild_id` = `gcc`.`guildid`;
...
FROM `ark_characters`.`guild_eventlog` `ge` JOIN `guild_character_map` `gcm` ON `gcm`.`source_guild_id` = `ge`.`guildid`;
...
FROM `ark_characters`.`guild_newslog` `gn` JOIN `guild_character_map` `gcm` ON `gcm`.`source_guild_id` = `gn`.`guildid`;
...
FROM `ark_characters`.`guild_rank` `gr` JOIN `guild_character_map` `gcm` ON `gcm`.`source_guild_id` = `gr`.`guildid`;
...
SELECT `icm`.`target_item_guid`, `icm`.`target_character_guid`, `itemEntry`, `cvs`.`slot`, 0, `randomProperty`, `suffixFactor` FROM `ark_characters`.`character_void_storage` `cvs` JOIN `item_character_map` `icm` ON `icm`.`source_item_guid` = `cvs`.`itemId` AND `icm`.`source_character_guid` = `cvs`.`playerGuid`;
	-- skip mail_items
