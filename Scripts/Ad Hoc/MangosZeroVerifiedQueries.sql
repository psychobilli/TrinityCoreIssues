...
-- Loremaster assistant with sp
CALL `zero_world`.`vanilla_loremaster_assistant`();
...
select qt.entry
    ...
    , ifnull(ef.completeCount, 0) as EliteForce
from zero_world.quest_template qt
  join questFactionResults qfr on qfr.entry = qt.entry
  left join (
	select quest
		, count(rewarded) as completeCount
    from zero_character.character_queststatus
...
where (qfr.FactionId = 0 and ifnull(ef.completecount, 0) < 5)
     or (qfr.FactionId = 1 and ifnull(ds.completecount, 0) < 5)
